...
class NotificationController extends BaseController
{
    ...
    public function index()
    {
        return $this->executeWithErrorHandling(function () {
            ...
            $role = intval($currentUser['role']);

            if (!$role || $role < 1) {
                $this->handleResponse(false, 'No se pudo determinar el rol del usuario.', [], 403);
                return;
            }
            ...
            $notifications = $this->notificationModel->getAllWithDetails($page, $limit, $all, $notificationType);

            $unreadCount = $this->notificationModel->getUnreadCount($notificationType);



            $this->handleResponse(true, 'Notificaciones obtenidas correctamente.', ['data' => $notifications, 'unread_count' => $unreadCount]);
    ...
    public function getUnreadCount()
    {
        return $this->executeWithErrorHandling(function () {
            ...
            $count = $this->notificationModel->getUnreadCount($notificationType);
            $this->handleResponse(true, 'Conteo de notificaciones no leídas obtenido correctamente.', ['unread_count' => $count]);
    ...
    public function markAsRead($id)
    {
        return $this->executeWithErrorHandling(function () use ($id) {
            ...
            // Verificar si la notificación existe y obtener su estado
            $notification = $this->notificationModel->getByIdWithDetails($notificationId);
            

            // Verificar si ya está marcada como leída
            if ($notification['is_read'] === 'READ') {
                $this->handleResponse(false, 'La notificación ya está marcada como leída', [], 400);
                return;
            }
            ...
            $resultado = $this->notificationModel->markAsRead($notificationId, $employeeId);
