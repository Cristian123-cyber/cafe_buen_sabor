...
class TablesController extends BaseController
{
    ...
    public function getQrToken($id)
    {
        return $this->executeWithErrorHandling(function () use ($id) {
            ...
            $table = $this->tableModel->find($tableId);
            if ($table && isset($table['qr_token']) && isset($table['table_status']) && $table['table_status'] !== 'INACTIVE' && $table['table_status'] !== 'DELETED') {
    ...
    public function delete($id)
    {
        return $this->executeWithErrorHandling(function () use ($id) {
            ...
            $table = $this->tableModel->updateTable($tableId, ['table_status' => 'DELETED', 'table_number' => null]);
            if ($table) {
