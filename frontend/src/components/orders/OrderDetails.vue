<template>
  <div v-if="order" class="order-detail-container">
    ...
    <div class="section-header">
      <div>
        <h3 class="order-id">Pedido #{{ order.id_order }} -  Mesa {{ order.table_number }}</h3>
    ...
    <div class="section">
      ...
      <ul class="product-list">
        <li v-for="product in order.products" :key="product.products_id_product" class="product-item">
          <img :src="product.product_image_url || 'https://via.placeholder.com/150'" alt="Imagen del producto"
            class="product-image" />
...
.order-detail-container {
  ...
  /* Añadimos p-1 para evitar que los focus ring se corten */
}
