...
const statusInfo = computed(() => {
  ...
  switch (status) {
    case 'PENDING':
      info.badgeClass += 'status-pending';
      break;
    case 'CONFIRMED':
      info.badgeClass += 'status-confirmed';
      break;
    case 'READY':
      info.badgeClass += 'status-ready';
      break;
    case 'DELIVERED':
      info.badgeClass += 'status-delivered';
      break;
    case 'CANCELLED':
      info.badgeClass += 'status-cancelled';
      break;
    default:
      info.badgeClass += 'status-unknown';
  }
  return info;
});
...
.order-card {
  @apply flex bg-white rounded-xl border border-gray-200 overflow-hidden 
...
.order-id {
  ...
  order: 1; /* Para reordenar visualmente */
}

.order-date {
  @apply text-sm text-gray-500 mb-4;
  order: 2; /* Para reordenar visualmente */
}
...
.order-action {
  @apply mt-auto pt-2; /* Empuja el botón hacia abajo */
}

/* --- Botón de Acción --- */
.btn-view-details {
  @apply flex items-center justify-center gap-2 w-full bg-accent text-white text-sm px-4 py-2.5 
         rounded-lg font-semibold transition-all duration-200 ease-in-out
         hover:bg-accent-dark focus:outline-none hover:shadow-lg;
}
...
/* No se necesita @media query porque el diseño base ya es para móviles y se adapta a pantallas más grandes */
/* Los prefijos `sm:` se encargan de los ajustes para pantallas mayores a 640px */
</style>